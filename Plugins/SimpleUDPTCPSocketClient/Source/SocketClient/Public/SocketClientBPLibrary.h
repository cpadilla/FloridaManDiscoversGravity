...
private:
	...
	FServerUDPConnectionThread* serverUDPConnectionThread = nullptr;
};
	...
	virtual uint32 Run() override {
		...
		if (socket != nullptr && socket->GetConnectionState() == ESocketConnectionState::SCS_Connected) {
			if (message.Len() > 0) {
				...
				int32 sent = 0;
				socket->Send((uint8*)utf8text, size, sent);
			}
			if (byteArray.Num() > 0) {
				int32 sent = 0;
...
public:
	...
	FServerUDPConnectionThread(USocketClientBPLibrary* socketClientP) :
		socketClient(socketClientP) {
	...
	virtual uint32 Run() override {
		...
		while (run) {
			...
			if (socket == nullptr || socket == NULL) {
				...
				if (socket == nullptr || socket == NULL) {
					run = false;
					UE_LOG(LogTemp, Error, TEXT("UE4 could not init a UDP socket. You can only create listening connections on local IPs. An external IP must be redirected to a local IP in your router. %s:%i"), *ip, port);
				}
				else {
			...
			if (run && (message.Len() > 0 || byteArray.Num() > 0)) {
				...
				if (bIsValid) {
					if (message.Len() > 0) {
						...
						socket->SendTo((uint8*)utf8text, size, sent, *addr);
					}
					if (byteArray.Num() > 0) {
						int32 sent = 0;
