...
FString USocketClientHandler::resolveDomain(FString serverDomain){
	...
	FString* cachedDomainPointer = USocketClientHandler::socketClientHandler->domainCache.Find(serverDomain);
	...
	if (lineCount == 4 && serverDomain.Len() <= 15 && serverDomain.Len() >= 7) {
		USocketClientHandler::socketClientHandler->domainCache.Add(serverDomain, serverDomain);
		return serverDomain;
	}
	...
	if (ResolveInfo->GetErrorCode() == 0) {
		...
		USocketClientHandler::socketClientHandler->domainCache.Add(serverDomain, adr);
		return adr;
	}
